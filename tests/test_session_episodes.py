...
def test_only_first_breakout_per_window_per_day_is_emitted(tmp_path) -> None:
    start = datetime(2026, 1, 5, 7, tzinfo=UTC)
    ...
    session_rows = [
        {
            ...
            "session_day_utc": "2026-01-05",
            "asia_width_bps": 10.0,
            "asia_breakout_side": side,
            "asia_breakout_distance_bps": 0.5 if side else 0.0,
        }
        for timestamp, side in zip(timestamps, (0, 1, 0, -1), strict=True)
    ]
    session_manifest, _ = _write_dataset(
        tmp_path / "sessions",
        session_rows,
    ...
    outcome_manifest, partition = _write_dataset(
        tmp_path / "outcomes",
        outcome_rows,
        {"dataset_id": "outcomes", "source_feature_dataset_id": "features"},
    )
    payload = json.loads(outcome_manifest.read_text(encoding="utf-8"))
    payload["horizon_partitions"] = {"300": str(partition)}
    outcome_manifest.write_text(json.dumps(payload), encoding="utf-8")
    result = write_session_episode_dataset(
        feature_manifest,
        session_manifest,
        structure_manifest,
